...
def determine_error_type(msg):
    msg = msg.lower()
    if any(x in msg for x in ["missing", "invalid key", "no auth", "no data — set claude", "no data — oauth failed", "no data — web api failed", "no logs/auth"]):
    ...
    if any(x in msg for x in ["connection", "timeout", "conn fail"]):
        return "timeout"
    if any(x in msg for x in ["parse error", "invalid response", "empty log", "no data", "no limits", "no quota", "no usage"]):
        return "parse_error"
...
for filepath in glob.glob("app/services/**/*.py", recursive=True):
    ...
    for i in range(len(lines)):
        ...
        if "error_card(" in line and "error_type=" not in line:
            ...
            import re
            match = re.search(r'(error_card\([^,]+,\s*"[^"]+",\s*)(f?"[^"]*?"|f?\'[^\']*?\')(.*?)\)', line)
            if match:
                ...
                if suffix.strip() == "":
                    ...
                    new_line = line[:match.start()] + f'{prefix}{msg_val}, error_type="{err_type}"{suffix})' + line[match.end():]
                    lines[i] = new_line
                    modified = True
            else:
