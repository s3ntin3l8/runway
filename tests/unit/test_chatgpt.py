...
def chatgpt_usage_response():
    ...
    return {
        ...
        "rate_limit": {
            "primary_window": {
                "used_percent": 25.5,
                "reset_at": int((datetime.now(timezone.utc) + timedelta(hours=3)).timestamp())
            }
        }
    }
...
class TestChatGPTCollectorDetailed:
    ...
    async def test_auth_priority_cookies_and_refresh(self, mock_http_client):
        ...
        with patch.dict('os.environ', {}, clear=True):
            with patch('os.path.exists', return_value=False):
                with patch('app.services.collectors.chatgpt.get_chatgpt_session_token', return_value=session_token):
                    ...
                    assert f"__Secure-next-auth.session-token={session_token}" in call_args.kwargs["headers"]["Cookie"]
    ...
    @pytest.mark.asyncio
    async def test_tier_detection_plus(self, mock_http_client, chatgpt_account_response, chatgpt_usage_response):
    ...
    async def test_collect_api_failure_fallback(self, mock_http_client):
        ...
        with patch.dict('os.environ', {'CHATGPT_OAUTH_TOKEN': 'token'}):
            with patch('app.services.collectors.chatgpt.glob.glob', return_value=["/fake/path/session.jsonl"]):
