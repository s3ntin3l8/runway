...
class OAuthBaseCollector(BaseCollector):
    ...
    def _persist_credentials(self, creds: Dict):
        ...
        if not settings.LOCAL_CREDENTIAL_SCRAPING_ENABLED:
            logger.info(f"Skipping {self.provider_name} token persistence (local credential scraping disabled)")
            return
            
        try:
            safe_write_json(self._credentials_path, creds)
            logger.info(f"Persisted {self.provider_name} refreshed tokens to {self._credentials_path}")
        except Exception as e:
    ...
    async def _get_valid_token(self, client: httpx.AsyncClient) -> Optional[str]:
        ...
        async with self._refresh_lock:
            ...
            if not self._can_attempt_refresh():
                logger.debug(f"{self.provider_name} token refresh backed off or terminal failure")
                return None
