...
class SafariBinaryCookieParser:
    ...
    def parse_page(data: bytes) -> List[Dict[str, Any]]:
        ...
        offsets = [struct.unpack('<I', data[8 + (i * 4):12 + (i * 4)])[0] for i in range(num_cookies)]
        
        for offset in offsets:
            cookie_data = data[offset:]
            if len(cookie_data) < 48: continue
...
def get_all_browser_cookies_paths() -> List[Dict[str, Any]]:
    ...
    chromium_variants = [
        ...
        {"name": "Edge", "darwin": "Microsoft Edge", "linux": [".config/microsoft-edge", ".config/microsoft-edge-beta", ".config/microsoft-edge-dev"], "win": "Microsoft/Edge/User Data"},
    ]
    ...
    for variant in chromium_variants:
        ...
        if system == "Darwin":
            ...
            base_dirs.append(Path(local_app_data) / variant["win"] if local_app_data else home / "AppData/Local" / variant["win"])
        else: # Linux
            for l_path in variant["linux"]:
                base_dirs.append(home / l_path)
                base_dirs.append(home / "snap" / variant["name"].lower().replace(' ', '-') / "common" / l_path)
        ...
        for base in base_dirs:
            ...
            for profile in profiles:
                for rel in [profile + "/Network/Cookies", profile + "/Cookies"]:
                    p = base / rel
                    if p.exists():
                        results.append({"browser": variant["name"], "type": "chromium", "path": p})

    # 2. Firefox
    ff_bases = []
    if system == "Darwin": ff_bases.append(home / "Library/Application Support/Firefox/Profiles")
    elif system == "Windows": ff_bases.append(home / "AppData/Roaming/Mozilla/Firefox/Profiles")
    else: # Linux
